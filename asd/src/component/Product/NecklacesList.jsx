 ...
 */
function getNecklacePrice(product) {
...
/**
 * getDiscountPercent
 * Returns the discount percentage if available; otherwise 0.
 */
...
// ----------------------------------------------------------------
// NecklacesList Component (JSX)
// ----------------------------------------------------------------

const NecklacesList = () => {
  const navigate = useNavigate();
  const FALLBACK_IMAGE =
    "https://dummyimage.com/300x300/ddd/000.png&text=No+Image";
  ...
  // Fetch Necklaces Data from Firestore (using the "Necklaces" collection)
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const snapshot = await getDocs(collection(db, "Necklaces"));
  ...
  useEffect(() => {
    ...
    updated = updated.filter((item) => {
      const price = getNecklacePrice(item);
    ...
    if (sortOption === "lowToHigh") {
      updated.sort((a, b) => getNecklacePrice(a) - getNecklacePrice(b));
    } else if (sortOption === "highToLow") {
      updated.sort((a, b) => getNecklacePrice(b) - getNecklacePrice(a));
    } else if (sortOption === "discount") {
      updated.sort(
        (a, b) => getDiscountPercent(b) - getDiscountPercent(a)
      );
    }
    // "bestSelling" – leave order unchanged
  ...
  return (
    <div className="max-w-screen-2xl mx-auto py-8 px-4">
      {/* Header */}
      <h1 className="text-2xl font-bold mb-2">NECKLACES</h1>
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentItems.map((product) => {
              // Retrieve the image from the imageUrls array.
              // Use the first URL if available and valid; otherwise, use the fallback.
              const mainImage =
                product.imageUrls &&
              ...
              const brandName = product.brand || "NECKLACE BRAND";

              return (
                <div
                  key={product.id}
                  className="border rounded-md bg-white shadow-sm hover:shadow-md transition p-3 relative"
                >
                  {/* Discount Badge */}
                  {hasSale && (
                    ...
                    {hasSale ? (
                      <>
                        <p className="text-red-600 font-bold text-sm">
                          {formatDollars(product.price.salePrice)}
                        </p>
                        <p className="text-gray-500 line-through text-xs">
                          {formatDollars(product.price.regularPrice)}
                        </p>
                      </>
                    ) : (
