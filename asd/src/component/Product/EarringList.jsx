 ...
 */
function getEarringPrice(product) {
...
const EarringList = () => {
  ...
  // Fetch Earring Data from Firestore (using the "Earring" collection)
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const snapshot = await getDocs(collection(db, "Earring"));
  ...
  useEffect(() => {
    ...
    updated = updated.filter((item) => {
      const price = getEarringPrice(item);
    ...
    if (sortOption === "lowToHigh") {
      updated.sort((a, b) => getEarringPrice(a) - getEarringPrice(b));
    } else if (sortOption === "highToLow") {
      updated.sort((a, b) => getEarringPrice(b) - getEarringPrice(a));
  ...
  return (
    <div className="max-w-screen-2xl mx-auto py-8 px-4">
      {/* Header */}
      <h1 className="text-2xl font-bold mb-2">EARRINGS</h1>
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentItems.map((product) => {
              ...
              const brandName = product.brand || "EARRING BRAND";
